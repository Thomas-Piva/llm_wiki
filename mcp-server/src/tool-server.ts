...
import { VAULT_TOOLS, isVaultTool, callVaultTool, loadVaultToolsConfig, type VaultToolsConfig } from "./vault-tools.js"
...
export function createToolServer(): Server {
  const client = new LlmWikiApiClient()
  const projectBinding = new McpProjectBinding()
  const vaultConfig: VaultToolsConfig | undefined = loadVaultToolsConfig()
